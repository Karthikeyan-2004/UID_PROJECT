...
export const HomePage = () => {
	...
	const onGenerateTrip = async () => {
		...
		setLoading(true);
		toast("We'll get you there soon!");
		...
		try {
			...
			// Clean up common JSON issues: remove unexpected line breaks and excess whitespace
			...
			toast("Failed to generate the trip. Please try again later.");
			setLoading(false);
		}
	};
								...
								<div className="flex flex-col space-y-10">
									<div className="space-y-2">
										...
										<div className="grid grid-cols-2 gap-4">
											{["Ooty", "Munnar","Kodaikanal","Coorg","Wayanad","Yercaud"].map((location) => (
												<div
													key={location}
													className={`p-4 border-2 rounded-lg cursor-pointer text-center hover:shadow-lg ${
														formData?.location === location
															? "bg-blue-700 text-white"
															: "border-blue-700"
													}`}
													onClick={() =>
														handleInputChanges("location", location)
													}
												>
													{location}
												</div>
